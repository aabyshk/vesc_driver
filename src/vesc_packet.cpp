...
VescPacketRotorPos::VescPacketRotorPos(boost::shared_ptr<VescFrame> raw) :
  VescPacket("RotorPos", raw)
{
}

double VescPacketRotorPos::rotor_pos() const
{
  int32_t v = static_cast<int32_t>((static_cast<uint32_t>(*(payload_.first + 1)) << 24) +
                                   (static_cast<uint32_t>(*(payload_.first + 2)) << 16) +
                                   (static_cast<uint32_t>(*(payload_.first + 3)) << 8) +
                                   static_cast<uint32_t>(*(payload_.first + 4)));

  return static_cast<double>(v)/100000.0;

}

REGISTER_PACKET_TYPE(COMM_ROTOR_POSITION, VescPacketRotorPos)

VescPacketRequestRotorPos::VescPacketRequestRotorPos() :
  VescPacket("RequestRotorPos", 1, COMM_ROTOR_POSITION)
{
  VescFrame::CRC crc_calc;
  crc_calc.process_bytes(&(*payload_.first), boost::distance(payload_));
  uint16_t crc = crc_calc.checksum();
...
VescPacketSetDetect::VescPacketSetDetect(disp_pos_mode mode):
  VescPacket("SetDetect", 2, COMM_SET_DETECT)
{

  int8_t v = static_cast<int8_t>(mode);

  *(payload_.first + 1) = static_cast<uint8_t>(static_cast<uint8_t>(v) & 0xFF);
  ...
  VescPacket("SetDetect", 3, COMM_SET_DETECT)
{
  ...
  uint16_t v = static_cast<uint16_t>(mode);

  *(payload_.first + 1) = static_cast<uint8_t>((static_cast<uint16_t>(v) >> 8) & 0xFF);
  *(payload_.first + 2) = static_cast<uint8_t>(static_cast<uint16_t>(v) & 0xFF);

  ROS_INFO_STREAM("SetDetect");
